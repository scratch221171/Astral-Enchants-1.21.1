...
public class AELanguageProviderJAJP extends LanguageProvider {
    ...
    protected void addTranslations() {
        ...
        add(AstralEnchant.MOD_ID + ".configuration." + AEConfig.LAST_STAND_REQUIRED_BASE_EXPERIENCE.getPath().getLast() , "ラストスタンドの必要経験値ポイント（基本値）");
        add(AstralEnchant.MOD_ID + ".configuration." + AEConfig.LAST_STAND_REQUIRED_BASE_EXPERIENCE.getPath().getLast() + ".tooltip", "レベル N のラストスタンドの消費経験値は「基本値 ÷ N」です。デフォルトでは約 34 レベルです。");
        add(AstralEnchant.MOD_ID + ".configuration." + AEConfig.LAST_STAND_IGNORE_BYPASSES_INVULNERABILITY_TAG.getPath().getLast(), "ラストスタンドが無敵貫通を無視");
        add(AstralEnchant.MOD_ID + ".configuration." + AEConfig.LAST_STAND_IGNORE_BYPASSES_INVULNERABILITY_TAG.getPath().getLast() + ".tooltip", "オフの場合、無敵貫通タグ（BYPASSES_INVULNERABILITY）を持つダメージ（/killや奈落など）ではラストスタンドは発動しません。");
        add(AstralEnchant.MOD_ID + ".configuration." + AEConfig.ESSENCE_OF_ENCHANT_INCLUDE_OVERLOAD_IN_CALCULATION.getPath().getLast(), "オーバーロードレベルをエンチャントの極意の総レベル計算に含める");
        ...
        add(AstralEnchant.MOD_ID + ".configuration." + AEConfig.INSTANT_TELEPORT_MAX_DISTANCE.getPath().getLast() + ".tooltip", "このエンチャントによるテレポートの距離の上限を指定します。");
        add(AstralEnchant.MOD_ID + ".configuration." + AEConfig.MITIGATION_PIERCING_ADDED_DAMAGE_TYPE_TAGS.getPath().getLast(), "軽減貫通で付与されるダメージタイプタグ");
